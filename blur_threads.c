...
struct blur_params blur_params_array[NUM_THREADS];
...
void *gaussian_blur(void *blur_args)
{
    ...
    int height = blur_data->height;
    float sigma = blur_data->sigma;
    int ksize = blur_data->blur_size;
